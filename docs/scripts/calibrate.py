...
from opentag_serial import MM_PER_DTU, collect_samples, open_port, request_info, save_calibration, send_and_expect

parser = argparse.ArgumentParser()
parser.add_argument("port", help="responder serial port")
parser.add_argument("--true-mm", type=float, required=True)
parser.add_argument("--samples", type=int, default=100)
parser.add_argument("--timeout", type=float, default=30, help="maximum collection time in seconds (default: 30)")
args = parser.parse_args()

if args.samples < 10 or not math.isfinite(args.true_mm) or args.true_mm <= 0 or not math.isfinite(args.timeout) or args.timeout <= 0:
    parser.error("--samples must be at least 10; --true-mm and --timeout must be finite and positive")
...
with open_port(args.port) as tag:
    ...
    samples, events = collect_samples(tag, count=args.samples, seconds=args.timeout)
    ...
    if not -(2 ** 31) <= new_offset < 2 ** 31:
        raise SystemExit("Calculated offset is outside the firmware's signed 32-bit range; check the reference distance.")
    send_and_expect(tag, f"CALIB {new_offset}", f"OK CALIB {new_offset}")
